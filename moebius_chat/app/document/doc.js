...
let nick, group;
// Base64 120-byte avatar attached to our outgoing chat (message.nick.avatar).
// Persisted in the renderer's localStorage; set via File ▸ Set Avatar from Canvas.
let avatar = "";
try { if (typeof localStorage !== "undefined") avatar = localStorage.getItem("avatar_chat_avatar") || ""; } catch (e) {}
let connection;
...
class Connection extends events.EventEmitter {
        ...
        if (!this.ready) {
            ...
            switch (type) {
                ...
                case actions.DRAW:
                    ...
                    break;
                case actions.CHAT:
                    // Render regardless of a local user entry: messages mirrored
    ...
    chat(text) {
        this.send(actions.CHAT, {nick, group, text, avatar});
        chat.chat(this.id, nick, group, text, Date.now(), avatar);
    }
    resize_cursors() {
...
class TextModeDoc extends events.EventEmitter {
    ...
    // Avatar attached to our outgoing chat messages (base64 120-byte block).
    get avatar() {return avatar;}
    set_avatar(base64) {
        avatar = base64 || "";
        try { if (typeof localStorage !== "undefined") localStorage.setItem("avatar_chat_avatar", avatar); } catch (e) {}
    }
